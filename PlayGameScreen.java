...
public class PlayGameScreen extends JPanel implements ActionListener {
   ...
   /** This list is used to stage obstacles before start of the game */
    ...
    and assigns a screen width, a screen height, and a boolean telling whether it is a splash screen or not. 
    */
    ...
    public PlayGameScreen(int sw, int sh) {
        ...
        initListObstacles();
        /* intialize the timer and start the timer. This will automatically call the action performed in periodical
        interval.
         */
    ...
    /** controls what is drawn on the JPanel by calling this method;
    ...
    /**
     whenever any obstacle run of screen and current available obstacle is only 3 on the obstacles list then
     create two more obstacle and add to the obstacles list
     */
   private void addListLogs() {
    ...
    /**
     move the dog downward trajectory till hit the 600 y location, user has to press space bar to move upward
     trajectory. It is called from paint component function. paint component is invoked from repaint call at
     the action performed function.action performed function is invoked from PlayGameScreen constructor. It calls swing.timer
     */
    ...
    /**
     move the obstacle in the screen from right left. It is called from paint component function. paint component is invoked from repaint call at
     the action performed function. action performed function is invoked from PlayGameScreen constructor. It calls swing.timer

     whenever x location of obstacle goes below the x location of the dog , then increment the score by 1. since both up and down obstacles cross the dog at the s
     same time , the score is always incremented by 2.  During display time in the paint component function, score will be divided by 2.
    ...
    /**
      Find the rectangle for dog and find the rectangle for each obstacle. If obstacle rectangle , intersects with dog rectangle
     set variable loop to false. This variable loop is used in action performed to stop the loop.
     */
